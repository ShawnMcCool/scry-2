...
- **Use `util:RemoveFolderEx` to remove the install directory on uninstall.** A plain `<RemoveFolder>` only removes a single empty directory — you'd need entries at every depth level of the Elixir release tree (lib/, erts-*/bin/, releases/X.Y.Z/, etc.). `util:RemoveFolderEx` schedules a recursive delete via a custom action and handles the whole tree in one step. Add it to a component in the install directory with a registry keypath.
