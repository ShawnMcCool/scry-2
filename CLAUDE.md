...
| **Buildability** | — | scores any card list against the collection — copies missing per card, wildcard cost, shortfall, and `:buildable`/`:craftable`/`:short`/`:incomplete` status. Owns no tables. One `CollectionPosition.current/1` read per page, then `score/4` per list; `assess/4` for a single list. Ownership summed across printings (`OwnedIdentity`). `collection_known?` distinguishes "owns none" from "no snapshot". See ADR-046 | Consumes `Cards` + `Collection` + `Economy`; no broadcast |
...
- **GenServer message protocols** — never use `:sys.replace_state` or direct `GenServer.call/cast` in tests to inspect or manipulate internal state. Always test through the module's public API ([ADR-009](decisions/architecture/2026-04-05-009-genserver-api-encapsulation.md)). Exception: `:sys.get_state/1` is acceptable as a **mailbox drain** — a synchronous no-op that ensures all pending casts have been processed before asserting. Use it only for that purpose, never to inspect returned state. GenServers with testable public logic should be tested. GenServers that are thin wrappers around external systems requiring real connections are not worth mocking.
