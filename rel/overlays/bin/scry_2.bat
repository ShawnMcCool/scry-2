...
rem When RELEASE_ROOT contains spaces (e.g. C:\Program Files\Scry2), cmd.exe splits
rem the token at the space and produces two "path specified" errors. Use for/f usebackq
rem which handles quoted paths with spaces correctly.
if not defined RELEASE_COOKIE (for /f "usebackq delims=" %%C in ("!RELEASE_ROOT!\releases\COOKIE") do set "RELEASE_COOKIE=%%C")
if not defined RELEASE_COOKIE (
  echo ERROR: RELEASE_COOKIE could not be read from "!RELEASE_ROOT!\releases\COOKIE" 1>&2
  echo        Verify the file exists and is readable. 1>&2
)

if defined SCRY_DEBUG (echo [scry_debug] RELEASE_COOKIE=[!RELEASE_COOKIE!] 1>&2)
