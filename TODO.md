...
## 0. Complete event ingestion coverage — **NEXT PRIORITY**

**Aspiration: every MTGA event type observed in Player.log has an explicit path through `Scry2.Events.IdentifyDomainEvents`.** Today only 2 of 6 defined domain events are wired, and many raw types silently fall through the catch-all clause. This work gates §4–§7 below.

### Current state

| Raw event type | Domain event | Status |
|---|---|---|
| `MatchGameRoomStateChangedEvent` (Playing) | `%MatchCreated{}` | done |
| `MatchGameRoomStateChangedEvent` (MatchCompleted) | `%MatchCompleted{}` | done |
| `GreToClientEvent` → `GameStateMessage` | `%GameCompleted{}` (stub exists) | next |
| `GreToClientEvent` → `ConnectResp` | `%DeckSubmitted{}` (stub exists) | next |
| `RankGetCombinedRankInfo`, `RankGetSeasonAndRankDetails` | new domain event TBD | todo |
| `AuthenticateResponse` | `%AuthenticateResponded{}` (new) | todo — unblocks self-user-id auto-detect (§3b) |
| `EventJoin`, `DeckUpsertDeckV2`, `EventSetDeckV2`, `EventEnterPairing`, `EventGetCoursesV2`, `EventGetActiveMatches`, `DeckGetDeckSummariesV2` | TBD per event | discovery |
| `ClientToGreuimessage`, `ClientToGremessage`, `GraphGetGraphState`, `QuestGetQuests`, `PeriodicRewardsGetStatus`, `StartHook`, `GetFormats` | likely none (UI/lobby noise) | discovery — explicit-ignore clause required, no silent fall-through |
| Draft events (raw types TBD) | `%DraftStarted{}`, `%DraftPickMade{}` (stubs exist) | blocked on draft fixtures |

### Order of work

1. Extract `Scry2.Events.GreMessageParser` — pure helper that flattens `greToClientEvent.greToClientMessages[]` into typed messages. Foundation for both `GameCompleted` and `DeckSubmitted`. Lives inside the Events context since it's part of the anti-corruption boundary.
2. Wire `%GameCompleted{}` end-to-end — IdentifyDomainEvents clause + `MatchListing.UpdateFromEvent` handler + `MatchListing.upsert_game!/1` + real fixture mined from Player.log.
3. Wire `%DeckSubmitted{}` end-to-end — fixture already committed as `gre_to_client_event_connect_resp.log`.
4. Rank events + `%AuthenticateResponded{}`.
5. Lobby event discovery pass — one fixture per remaining observed type, decide domain-event vs explicit-ignore clause for each.
6. Noise discovery pass — the `ClientToGre*` / `GraphGetGraphState` / quest / reward types. Likely all explicit-ignore, but each needs a fixture and a documented decision.
7. Drafts — unblock when the user runs a draft session with detailed logs enabled.

Each step follows the recipe in §3b ("Pattern for adding a new domain event type").

### Definition of done

- Every raw event type observed in the user's current `Player.log` has either a IdentifyDomainEvents clause producing a domain event OR an explicit fall-through clause with a `# discarded because …` comment. No silent skips.
- Each IdentifyDomainEvents clause backed by a real fixture under `test/fixtures/mtga_logs/` (ADR-010 append-only).
- A diagnostic — e.g. `Scry2.Events.unknown_event_types/0` — reports zero unrecognized raw types against the current log.
- `mix precommit` green, zero warnings.
...
**Thin slice (first session):** direct mapper-based path for match creation. `MtgaLogIngestion.get_event!/1`, `MatchListing.upsert_game!/1`, ingester error handling, `Scry2.MatchListing.EventMapper`, ingester dispatch wired, `mtga_self_user_id` config, 185 tests.
...
- **New bounded context** `Scry2.Events` (table: `domain_events`, append-only): holds the domain event log plus IdentifyDomainEvents (anti-corruption layer), IngestRawEvents, and the `Event` protocol.
- **Domain event catalog** under `lib/scry_2/events/`: `MatchCreated`, `MatchCompleted` (both wired); `GameCompleted`, `DeckSubmitted`, `DraftStarted`, `DraftPickMade` (struct + protocol defined, no IdentifyDomainEvents clause yet).
- **`Scry2.Events.IdentifyDomainEvents`** (pipeline stage 07, pure) — the ONLY place MTGA wire format is understood. Consumes `%MtgaLogIngestion.EventRecord{}` + `self_user_id`, returns list of domain event structs. Handles `MatchGameRoomStateChangedEvent` with `Playing` → `%MatchCreated{}` and `MatchCompleted` → `%MatchCompleted{}` (including win/loss derivation from `finalMatchResult.resultList`).
- **`Scry2.Events.IngestRawEvents`** (pipeline stage 08) — GenServer, subscribes to `mtga_logs:events`, runs IdentifyDomainEvents, persists via `Events.append!/2`, marks raw events processed. Single subscriber of the raw topic.
- **Event handlers** — `Scry2.MatchListing.UpdateFromEvent` and `Scry2.DraftListing.UpdateFromEvent` (pipeline stage 09) — GenServers subscribing to `domain:events`. MatchListing.UpdateFromEvent handles `%MatchCreated{}` and `%MatchCompleted{}` via `MatchListing.upsert_match!/1`. DraftListing.UpdateFromEvent has empty `@claimed_slugs` pending draft fixtures.
- **Rebuild tooling** — `Scry2.Events.replay_projections!/0` (drop projections, replay domain event log) and `Scry2.Events.retranslate_from_raw!/0` (drop domain events, re-run IdentifyDomainEvents from raw log).
- **Deleted** — `Scry2.MatchListing.EventMapper`, `Scry2.MatchListing.Ingester`, `Scry2.DraftListing.Ingester` and their tests. Replaced by IdentifyDomainEvents + UpdateFromEvent infrastructure.
...
2. **Add a IdentifyDomainEvents clause** — in `lib/scry_2/events/identify_domain_events.ex`, add a `translate/2` head for the relevant raw MTGA event type that produces the struct. Use real fixtures from `test/fixtures/mtga_logs/` (ADR-010).
3. **Add a rehydration clause** — in `Scry2.Events.get/1` to deserialize the persisted payload back into the struct.
4. **Add an UpdateFromEvent handler** — in the relevant handler (`MatchListing.UpdateFromEvent`, `DraftListing.UpdateFromEvent`, or a new one) pattern-match on the struct type and call the appropriate upsert. Add the slug to `@claimed_slugs`.
5. **Test the slice** — struct test, IdentifyDomainEvents test with fixture, UpdateFromEvent test, end-to-end IngestRawEvents test.

### Match completion / final results — **DONE** (event-sourced refactor session)

`%Scry2.Events.MatchCompleted{}` wired end-to-end. IdentifyDomainEvents produces it from `MatchGameRoomStateChangedEvent` with `stateType: MatchCompleted`, computing `won` from `finalMatchResult.resultList[].winningTeamId` vs the self team (from `reservedPlayers[]`). UpdateFromEvent enriches the existing `matches_matches` row via `upsert_match!/1` (idempotent by `mtga_match_id`).
...
- IdentifyDomainEvents clause for raw `GreToClientEvent`: run through GreMessageParser, emit `%GameCompleted{}` for each GameStateMessage with `matchState: MatchState_GameComplete`.
- Add `game_completed` handler to `MatchListing.UpdateFromEvent`, call `MatchListing.upsert_game!/1`.
...
- IdentifyDomainEvents clause for raw `GreToClientEvent`: find ConnectResp messages, transform flat arrays into `[%{arena_id: _, count: _}]` aggregated shape, emit `%DeckSubmitted{}`.
...
- Add `deck_submitted` handler to `MatchListing.UpdateFromEvent`, call `MatchListing.upsert_deck_submission!/1`.
...
- Add IdentifyDomainEvents clauses that produce `%DraftStarted{}` and `%DraftPickMade{}`.
- Wire `Scry2.DraftListing.UpdateFromEvent.@claimed_slugs` (currently empty), add handlers that call `DraftListing.upsert_draft!/1` and `DraftListing.upsert_pick!/1`.
...
`parse_timestamp/1` in `extract_events_from_log.ex` currently tags all timestamps as `"Etc/UTC"` — MTGA writes them as local time with no zone annotation, so there's drift. Fix by either sniffing the OS timezone at startup or adding a `mtga_log_timezone = "auto"` config key. Ordering/relative comparisons work correctly regardless; only absolute display timestamps are wrong.

### Self-user-id auto-detection

Currently the user can set `mtga_logs.self_user_id` in `config.toml`, otherwise IdentifyDomainEvents falls back to `systemSeatId: 1`. Better: IdentifyDomainEvents emits a synthetic `%Scry2.Events.AuthenticateResponded{client_id: ...}` event on session start, the `IngestRawEvents` stores the current client_id in a lightweight ETS table or GenServer, and IdentifyDomainEvents reads from there instead of Config on subsequent events. Removes the need for user configuration and handles the case where the user has multiple MTGA accounts.
...
Note: `%GreMessage{}` is a private parsing helper, NOT a domain event. It doesn't implement `Scry2.Events.Event` and is never persisted. It just exists to untangle the deep nesting before IdentifyDomainEvents builds real domain events from it.
...
> **Blocked on §0 (complete event ingestion coverage).**
...
- Verify Plan Risk #1 — run `SeventeenLands.run()` against the real file and confirm whether `arena_id` is or isn't present. If 17lands has added it, this item collapses. If not, Scryfall backfill is required and is the only way log events can be joined to card metadata.
...
- `Scry2.Workers.ScryfallBackfillWorker` (Oban) — runs after `PeriodicallyUpdateCards`.
...
3. `Scry2.Cards.SeventeenLands.run()` live — confirm > 20 000 rows and spot-check named cards in `/cards`.
