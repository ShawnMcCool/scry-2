...
/// Cached walker entry point for the server environment record
/// (spike 23). Same discovery-cache pattern as the other
/// `walk_*_cached` wrappers.
///
/// `Ok(None)` when `PAPA._instance` is null (pre-bootstrap MTGA) or
/// when the chain otherwise hops short. `Ok(Some(EnvironmentInfo {...}))`
/// otherwise. Reads only public fields — see `walker::environment`
/// module docs for the security boundary.
pub fn walk_environment_cached<F>(
    pid: u32,
    maps: &[MapEntry],
    read_mem: F,
) -> Result<Option<crate::walker::environment::EnvironmentInfo>, WalkError>
where
    F: Fn(u64, usize) -> Option<Vec<u8>> + Copy,
{
    let offsets = MonoOffsets::mtga_default();
    ...
    Ok(crate::walker::environment::from_papa_singleton(
        &offsets,
        papa_singleton,
        &papa.class_bytes,
        read_mem,
...
/// Cached walker entry point for cosmetics counts (spike 22). Same
/// discovery-cache pattern as the other `walk_*_cached` wrappers.
///
/// `Ok(None)` when `PAPA._instance` is null (pre-login MTGA) or the
/// chain otherwise hops short. `Ok(Some(CosmeticsSummary { ... }))`
/// otherwise; per-sub-chain failures collapse into zero counts on
/// the returned struct rather than killing the read.
pub fn walk_cosmetics_cached<F>(
    pid: u32,
    maps: &[MapEntry],
    read_mem: F,
) -> Result<Option<crate::walker::cosmetics::CosmeticsSummary>, WalkError>
where
    F: Fn(u64, usize) -> Option<Vec<u8>> + Copy,
{
    let offsets = MonoOffsets::mtga_default();
    ...
    Ok(crate::walker::cosmetics::from_papa_singleton(
        &offsets,
        papa_singleton,
        &papa.class_bytes,
        read_mem,
