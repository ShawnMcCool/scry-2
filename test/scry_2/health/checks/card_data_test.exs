defmodule Scry2.Health.Checks.CardDataTest do
  ...
  describe "synthesized_present/1" do
    test "error with enqueue fix when count is 0" do
      check = CardData.synthesized_present(0)
    ...
    test "ok when cards are present" do
      check = CardData.synthesized_present(24_567)
      assert %Check{status: :ok} = check
      assert check.summary =~ "24567"
    end
  end

  describe "synthesized_fresh/2" do
    test "ok within 7 days" do
      now = ~U[2026-04-11 12:00:00Z]
      updated_at = ~U[2026-04-08 12:00:00Z]
      check = CardData.synthesized_fresh(updated_at, now)
      assert %Check{status: :ok} = check
    ...
    test "warning when updated_at is nil" do
      check = CardData.synthesized_fresh(nil)
      assert %Check{status: :warning, fix: :enqueue_synthesis} = check
    end
  end
