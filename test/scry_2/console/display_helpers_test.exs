defmodule Scry2.Console.EntryViewTest do
  use ExUnit.Case, async: true

  alias Scry2.Console.{Entry, Filter, EntryView}
  ...
  describe "known_components/0" do
    test "returns app components first, then framework components" do
      components = EntryView.known_components()
    ...
    test "app and framework accessors reflect the split" do
      assert EntryView.app_components() == [
               :watcher,
               :parser,
               :ingester,
               :importer,
      ...
      assert EntryView.framework_components() == [:phoenix, :ecto, :live_view]
    end
  end

  describe "format_timestamp/1" do
    test "formats HH:MM:SS.mmm" do
      assert EntryView.format_timestamp(~U[2026-04-05 14:23:45.123000Z]) == "14:23:45.123"
    end

    test "zero-pads single digit fields" do
      assert EntryView.format_timestamp(~U[2026-04-05 04:03:02.001000Z]) == "04:03:02.001"
    end
  end

  describe "level_color/1" do
    test "returns the right class for every level" do
      assert EntryView.level_color(:error) == "text-error"
      assert EntryView.level_color(:warning) == "text-warning"
      assert EntryView.level_color(:info) == "text-info"
      assert EntryView.level_color(:debug) == "text-base-content/60"
    end
  end

  describe "component_label/1 and component_badge_class/1" do
    test "component_label converts atoms to strings" do
      assert EntryView.component_label(:ingester) == "ingester"
      assert EntryView.component_label(nil) == "system"
    end

    test "component_badge_class returns a valid daisy badge class" do
      class = EntryView.component_badge_class(:ingester)
  ...
  describe "format_line/1 and format_lines/1" do
    ...
    test "format_lines reverses newest-first entries to chronological order" do
      ...
      output = EntryView.format_lines(newest_first)
  ...
  describe "level_button_class/2" do
    test "returns btn-active when levels match" do
      ...
      assert EntryView.level_button_class(filter, :warning) == "btn-active"
    end

    test "returns empty string otherwise" do
      filter = %Filter{level: :info}
      assert EntryView.level_button_class(filter, :error) == ""
    end
  end

  describe "entry_search_text/1" do
    test "lowercases the message" do
      assert EntryView.entry_search_text(entry(%{message: "Hello WORLD"})) == "hello world"
    end
  end

  describe "pause_button_label/1" do
    test "true -> resume, false -> pause" do
      assert EntryView.pause_button_label(true) == "resume"
      assert EntryView.pause_button_label(false) == "pause"
    end
  end
  ...
  describe "only_search_changed?/2" do
    test "true when only search differs" do
      ...
      b = %{a | search: "foo"}
      assert EntryView.only_search_changed?(a, b)
    end

    test "false when level also differs" do
      a = Filter.new_with_defaults()
      b = %{a | search: "foo", level: :error}
      refute EntryView.only_search_changed?(a, b)
    end

    test "false when searches are equal" do
      a = Filter.new_with_defaults()
      refute EntryView.only_search_changed?(a, a)
    end
  end
end
