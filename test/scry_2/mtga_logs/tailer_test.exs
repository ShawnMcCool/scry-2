defmodule Scry2.MtgaLogs.TailerTest do
  use ExUnit.Case, async: true

  alias Scry2.MtgaLogs.Tailer
  ...
  test "read_since/2 returns only the new bytes after a partial read", %{path: path} do
    ...
    {:ok, result} = Tailer.read_since(path, offset)
  ...
  test "read_since/2 flags rotation when file shrinks", %{path: path} do
    ...
    {:ok, result} = Tailer.read_since(path, 100)
  ...
  test "read_since/2 reports error for missing file" do
    assert {:error, _} = Tailer.read_since("/does/not/exist/scry2-tailer.log", 0)
  end
end
