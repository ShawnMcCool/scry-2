defmodule Scry2.MtgaLogIngestion.LocateLogFileTest do
  ...
  describe "resolve/1 with explicit candidate list" do
    test "returns the first existing file in the candidate list" do
      ...
      assert {:ok, ^tmp_b} =
               LocateLogFile.resolve(candidates: [tmp_a, tmp_b])
    ...
    test "override wins over candidate list" do
      ...
      assert {:ok, ^tmp} =
               LocateLogFile.resolve(override: tmp, candidates: ["/does/not/exist.log"])
    ...
    test "override is rejected when the file doesn't exist" do
      ...
      assert {:error, :not_found} =
               LocateLogFile.resolve(override: "/nowhere/Player.log", candidates: [fallback])
  ...
  describe "default_candidates/0" do
    test "contains the Steam Proton flatpak path first" do
      [first | _] = LocateLogFile.default_candidates()
