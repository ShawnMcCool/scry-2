...
151 unit tests passing in `native/scry2_collection_reader/`,
...
| # | Module | Scope | Notes |
|---|---|---|---|
| 1 | Outer `chain.rs` layers | `from_papa_class(class_addr, domain_addr, …class_bytes…)` — uses `vtable::static_storage_base` + `field::find_by_name("_instance")` to get the PAPA singleton, then walks `<InventoryManager>k__BackingField` (instance) → `_inventoryServiceWrapper` (instance) → delegates to existing `from_service_wrapper`. | Pure orchestration on top of `vtable.rs` + `field.rs`. Adds 2 new top-level helpers: `read_static_pointer` and `from_papa_class`. |
| 2 | Top-level `walk_collection(pid)` in `chain.rs` (or new `walker::run`) | Stitches everything: `list_maps` → find `mono-2.0-bdwgc.dll` base + bytes → `domain::find_root_domain` for live `MonoDomain *` → `image_lookup` for Core/Assembly-CSharp → `class_lookup` for the five classes → `from_papa_class` → `WalkResult`. | Returns `Result<WalkResult, WalkError>`. The error enum should distinguish the failure mode (no MTGA, can't find symbol, can't find class, dict cap exceeded, etc.) so the Elixir `Scry2.Collection.Reader` can route loudly. |
| 3 | `build_hint.rs` | Read MTGA's build GUID from `<MTGA root>/MTGA_Data/boot.config` — a plain key=value file on disk, **not** on the PAPA chain. Adds `mtga_build_hint` to `WalkResult`. | Trivial once the disk-path discovery (parse `/proc/<pid>/maps` for the MTGA install dir) is solved. |
