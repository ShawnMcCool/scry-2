...
- Cosmetics inventory (pets, sleeves, avatars, alt arts, emotes) — **✅ shipped.** `Scry2.Collection`'s walker reads `_availableCosmetics` and `_playerOwnedCosmetics` `CosmeticsClient` instances and stamps per-category counts on each snapshot via `cosmetics_json`. Rendered as a Cosmetics card on `/economy` — six rows (Alt arts, Avatars, Pets, Sleeves, Emotes, Titles) with owned / total + a per-row progress bar. The walker also reads the `ClientVanitySelectionsV3` slot strings (avatar / cardBack / pet / title) for future surface; not yet displayed. The `Titles` list reads `0 / 0` on the captured build — MTGA likely stores titles via the separate `_titlesCatalog` field (NULL at spike capture); follow-up to drill that.
