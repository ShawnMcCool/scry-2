...
- Cosmetics inventory (pets, sleeves, avatars, alt arts, emotes) — **reader+, spike-ready.** Spike 22 captured the chain — `PAPA._instance.<CosmeticsProvider>k__BackingField → CosmeticsProvider → {_availableCosmetics, _playerOwnedCosmetics, _vanitySelections}`. Each `CosmeticsClient` exposes six `List<T>` collections (ArtStyles, Avatars, Pets, Sleeves, Emotes, Titles); diffing available vs owned gives the per-category "owned / total" counts. Currently-equipped read from `ClientVanitySelectionsV3.{avatarSelection, cardBackSelection, petSelection, emoteSelections, titleSelection}`. See `mtga-duress/experiments/spikes/spike22_papa_managers/FINDING.md`.
