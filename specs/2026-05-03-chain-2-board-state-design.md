...
**Implementation deviation (discovered 2026-07-21, resolved 2026-07-22):** sub-project 4 as shipped (`walker/card_holder.rs::read_battlefield_arena_ids`) did **not** do the region/stack drill-down described above. It walks the simpler `BattlefieldLayout._unattachedCardsCache` — a single flat `List<DuelScene_CDC>` — instead of the per-seat `_local*Region`/`_opponent*Region` fields, and that cache is not split by player (confirmed live: `PlayerTypeMap` holds exactly one battlefield holder, keyed `0`). Rather than switching to the region traversal planned here, the fix went a different route: each card carries its own owner (`CardInstanceData.Owner → MtgPlayer.ClientPlayerEnum`), so `read_battlefield_cards` resolves seat per card instead of per holder. See `plans.md` section D for the full writeup.
