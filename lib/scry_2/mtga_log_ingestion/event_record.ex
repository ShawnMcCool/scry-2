defmodule Scry2.MtgaLogIngestion.EventRecord do
  @moduledoc """
  Ecto schema for persisted raw log events (table `mtga_logs_events`).

  Named `EventRecord` to distinguish from `Scry2.MtgaLogIngestion.Event`, which is
  the in-memory struct produced by `Scry2.MtgaLogIngestion.ExtractEventsFromLog`. Every
