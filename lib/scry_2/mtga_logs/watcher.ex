defmodule Scry2.MtgaLogs.Watcher do
  @moduledoc """
  ...
  3. On `:modified` events we read the new byte range, run it through
     `EventParser`, persist each event via `Scry2.MtgaLogs.insert_event!/1`,
  ...
  * **Permission / I/O errors.** Logged, status broadcast, then retry
  ...
  defp resolve_path(_state), do: PathResolver.resolve()

  defp start_watching(state, path) do
    cursor = MtgaLogs.get_cursor(path)
  ...
  defp drain_file(%{path: path, offset: offset} = state) when is_binary(path) do
    case Tailer.read_since(path, offset) do
      ...
      {:ok, %{bytes: bytes, new_offset: new_offset, rotated?: rotated, inode: inode}} ->
        ...
        events = EventParser.parse_chunk(bytes, path, base_offset)
  ...
  defp persist_event(%Scry2.MtgaLogs.Event{} = event) do
