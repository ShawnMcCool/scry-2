defmodule Scry2.Ranks.Format do
  @moduledoc """
  Compose human-readable rank strings from MTGA's class+tier pair.

  Used by the matches, decks, and ranks consumers to produce values like
  `"Gold 3"` or `"Mythic"` from the raw class/tier fields surfaced by the
  log translator and the memory-observation enricher.
  ...
  **Mythic special case:** MTGA emits `tier: 1` for Mythic-tier players,
  ...
  is encoded in the placement / percentile fields, displayed separately
  by `Scry2Web.Components.RankBadge`). This module collapses any
  Mythic+tier input to just `"Mythic"`.
